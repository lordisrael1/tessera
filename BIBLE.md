...
| 5.4 | Prefill programs at bucketed lengths {128, 512, 2048} | **Chunks of ≤ 42 tokens** | Not a scope cut, a hardware limit: activations are ~81 KB/token against an 8 MB scratchpad shared with two weight banks, so 128 does not fit and no schedule makes it fit. `max_prefill_tokens()` computes the bound; `lower_prefill` takes a `base` so chunks compose, which is chunked prefill as production systems do it. Measured payoff at M=32: **31.7× fewer cycles than 32 decode steps, bit-exact**. `docs/04` §5. |
| 5.2 | `V_SOFTMAX // spm src, rows, cols` | Implemented exactly that, plus **zeroing past each row's causal boundary** | The mask has to live in the data because the AV matmul reads the full rectangle. Zeroed entries contribute `0.0`, which a double accumulator ignores, so the reference can sum only the valid prefix and still match bit for bit. |
